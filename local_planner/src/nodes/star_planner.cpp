...
void StarPlanner::buildLookAheadTree() {
  ...
  RCLCPP_INFO(star_planner_logger_, "\033[0;35m[SP]Tree (%lu nodes, %lu path nodes, %lu expanded) calculated in %2.2fms.\033[0m", tree_.size(),
  ...
  for (int j = 0; j < path_node_positions_.size(); j++) {
    RCLCPP_DEBUG(star_planner_logger_, "\033[0;35m[SP] node %i : [ %f, %f, %f]\033[0m", j, path_node_positions_[j].x(),
